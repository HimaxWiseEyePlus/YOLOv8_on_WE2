...
void diounms_sort(std::forward_list<detection> &dets, int classes, float thresh)
{
    ...
    **/
    ...
    bool ClassifyImageHandler(ApplicationContext& ctx, uint32_t imgIndex, bool runAll)
    {
        ...
        const uint32_t nCols = inputShape->data[arm::app::YoloFastestRelu6HIMAX::ms_inputColsIdx];
        const uint32_t nRows = inputShape->data[arm::app::YoloFastestRelu6HIMAX::ms_inputRowsIdx];
        const uint32_t nChannels = inputShape->data[arm::app::YoloFastestRelu6HIMAX::ms_inputChannelsIdx];
        ...
        do {
            ...
            info("Running YOLO FASTEST HIMAX model inference on image %" PRIu32 " => %s\n", ctx.Get<uint32_t>("imgIndex"),
