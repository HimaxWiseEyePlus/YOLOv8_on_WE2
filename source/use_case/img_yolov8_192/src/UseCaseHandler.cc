        ...
        do {
            ...
            for(int dims_cnt_2 = 0; dims_cnt_2 < output->dims->data[2]; dims_cnt_2++)
            {
                for(int dims_cnt_1 = 0; dims_cnt_1 < output->dims->data[1]; dims_cnt_1++)
                {
                    ...
                    outputs_data[dims_cnt_2][dims_cnt_1] = deq_value;
                }
            }

            for(int dims_cnt_2=0;dims_cnt_2<output->dims->data[2];dims_cnt_2++)
            {
                float maxScore = outputs_data[dims_cnt_2][4];// the first four indexes are bbox information
                int maxClassIndex = 0;
                for(int class_id_cnt = 4;class_id_cnt < output->dims->data[1];class_id_cnt++)
                {
                    /***
                     * find maximum Score and correspond Class id
                     * **/
                    if(maxScore<outputs_data[dims_cnt_2][class_id_cnt])
                    {
                        maxScore=outputs_data[dims_cnt_2][class_id_cnt];
                        maxClassIndex = class_id_cnt-4;
                    }
                }
                // if (maxScore >= 0.25)
                if (maxScore >= modelScoreThreshold)
                {
                    ...
                    bbox.x = outputs_data[dims_cnt_2][0] - (0.5 * outputs_data[dims_cnt_2][2]);
                    bbox.y = outputs_data[dims_cnt_2][1] - (0.5 * outputs_data[dims_cnt_2][3]);
                    bbox.w = outputs_data[dims_cnt_2][2];
                    bbox.h = outputs_data[dims_cnt_2][3];
